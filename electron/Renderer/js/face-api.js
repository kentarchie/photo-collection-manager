  ...
  ***************************************************************************** */
  ...
  var DrawBox = /** @class */ (function () {
      function DrawBox(box, options) {
	      console.log('KCA: face-api.DrawBox(): START');
          if (options === void 0) { options = {}; }
          this.box = new Box(box);
          this.options = new DrawBoxOptions(options);
      }
      DrawBox.prototype.draw = function (canvasArg) {
	      console.log('KCA: face-api.DrawBox.draw START');
	      ...
	      console.log('KCA: face-api.DrawBox.draw boxColor: ',boxColor);
	      console.log('KCA: face-api.DrawBox.draw lineWidth: ',lineWidth);
	      ...
	      console.log('KCA: face-api.DrawBox.draw DONE');
      };
      return DrawBox;
  }());

  function drawDetections(canvasArg, detections) {
	    console.log('KCA: drawDetections: FIRST: canvasArg :', JSON.stringify(canvasArg.getBoundingClientRect(),null,'\t'));
	  console.log('KCA: face-api.drawDetections; START');
	  ...
	  console.log('KCA: face-api.drawDetections; score:',score);
          var box = det instanceof FaceDetection
              ? det.box
              : (isWithFaceDetection(det) ? det.detection.box : new Box(det));
          var label = score ? "" + round(score) : undefined;
	  console.log('KCA: face-api.drawDetections; label:',label);
          new DrawBox(box, { label: label }).draw(canvasArg);
      });
	    console.log('KCA: drawDetections: LAST: canvasArg :', JSON.stringify(canvasArg.getBoundingClientRect(),null,'\t'));
	  console.log('KCA: face-api.drawDetections; DONE');
  }
  ...
  function matchDimensions(input, reference, useMediaDimensions) {
	  console.log('KCA: matchDimensions: START: canvas.width before = ', input.width);
	...
	var pos = input.getBoundingClientRect();
	  ...
	  console.log('KCA: matchDimensions: START: canvas.width after = ', input.width);
      return { 'width': width, 'height': height, 'top': pos['top'], 'left': pos['left'] };
  }
